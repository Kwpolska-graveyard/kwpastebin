...
try
{
   ...
   $stmt = $pdo -> prepare('SELECT code, language, desc FROM `'.$dbtbl.'` WHERE `timestamp` = ?');
   $stmt->execute(array($_GET['id']));
   $obj = $stmt->fetch(PDO::FETCH_OBJ);
   $timestamp = substr($_GET['id'], 0, -2);
   echo '<a href="./plain.php?id='.$_GET['id'].'">plaintext</a> &mdash; <a href="./dl.php?id='.$_GET['id'].'">download</a> &mdash; added '.date('F jS, Y \at h:i:s A T', $timestamp).' &mdash; <em>'.$obj->desc.'</em> &mdash; hilighted by <a href="http://qbnz.com/highlighter/">GeSHi</a><br>';
